...
mod tests {
    ...
    #[test]
    fn add_column_rejects_nested_current_default_metadata() {
        ...
        .expect_err("ADD COLUMN cannot introduce a current default");

        assert!(error.to_string().contains("payload.code"));
        assert!(error.to_string().contains("CURRENT_DEFAULT"));
    }
